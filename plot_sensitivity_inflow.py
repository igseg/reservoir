...
def model_inflow(prev, t,scale):
    return coefs_inflow[0] * scale + coefs_inflow[1]* prev + coefs_inflow[2] * (sin((t+coefs_inflow[3])*2*pi/12))

def model_storage(model_inflow, prev_inflow , model_evaporation, t, prev_storage, outflow,scale):
    inflow      = model_inflow(prev_inflow, t, scale)
    evaporation = model_evaporation(t)

    return prev_storage + 2.592 * inflow - evaporation -  2.592 * outflow

def next_storage(model_inflow, prev_inflow , model_evaporation, t, prev_storage, outflow, model_storage, scale, shock_n):
...
def saving_scale_results(policy = None, V0 = None, scale = None):
    policy.tofile(f'Results/policy_2_mr_model_inflow_{scale}.txt')
    V0.tofile(f'Results/V0_2_mr_model_inflow_{scale}.txt')

def load_scale_results(scale):
    mr_file = open(f'Results/policy_2_mr_model_inflow_{scale}.txt')
    loaded_policy = np.fromfile(mr_file)
    loaded_policy = loaded_policy.reshape((S,N,M))
    mr_file.close()

    V0_mr = open(f'Results/V0_2_mr_model_inflow_{scale}.txt')
...
def model_inflow_sensib(prev, t, scale = 1):
    return coefs_inflow[0] * scale + coefs_inflow[1]* prev + coefs_inflow[2] * (sin((t+coefs_inflow[3])*2*pi/12))

def model_storage_sensib(model_inflow_sensib, prev_inflow , model_evaporation, t, prev_storage, outflow, scale):
    inflow      = model_inflow_sensib(prev_inflow, t, scale)
    evaporation = model_evaporation(t)
    return prev_storage + 2.592 * inflow - evaporation -  2.592 * outflow
...
def pi_fun(storage, inflow, t, P, q, model_inflow_sensib, model_evaporation, scale):

    expected_storage = model_storage_sensib(model_inflow_sensib, inflow , model_evaporation, t, storage, q, scale) # this may not be needed if forecast is done.
    return eta * q * ((z(storage, elev_stor) + z(expected_storage,elev_stor))/2 - zd) * P

def pi_call_0(storage, inflow, t, P, q, scale):
    if storage > 2e4 or storage < 3e4: # this must be wrong
        return pi_fun(storage, inflow, t, P, q, model_inflow_sensib, model_evaporation, scale)
    else:
        return -1e7
...
def future_payments(V,A,shocks_storage, shocks_price, input_next_storage, input_next_price, price_grid, storage_grid,boundaries,ts_forecast,scale):
    K = len(shocks_storage)
    J = len(shocks_price  )

    future_payment = np.zeros((K,J))

    future_storage = next_storage(*input_next_storage,scale,  shocks_storage)
...
def action_value(input_next_storage, input_next_price, V0, A, shocks_storage, shocks_prices, price_grid, storage_grid, boundaries, ts_forecast, scale):
    ...
    next_state_value, under_storage, over_storage = future_payments(V0[s_next],A, shocks_storage, shocks_prices,
                                                            ...
                                                            price_grid, storage_grid, boundaries, ts_forecast,scale)
    ...
    if under_storage or over_storage:
        ...
        return pi_call_0(storage, inflow, current_time, price, q, scale) + next_state_value
...
@jit
def pi_fun_numba(storage, inflow, t, P, q, scale):
    inflow  = coefs_inflow[0] * scale + coefs_inflow[1]* inflow + coefs_inflow[2] * (np.sin((t+coefs_inflow[3])*2*np.pi/12))
...
@jit
def next_storage_numba(prev_inflow, t, prev_storage, outflow, shock_n, scale):
    inflow  = coefs_inflow[0] * scale + coefs_inflow[1]* prev_inflow + coefs_inflow[2] * (sin((t+coefs_inflow[3])*2*pi/12))
...
def future_payments_numba(V,A,shocks_storage, shocks_price, inflow, current_time,
                          storage, q, price, price_grid, storage_grid,boundaries, scale):

    #future_payment = np.zeros((K,J))

    future_storage = next_storage_numba(inflow, current_time, storage, q, shocks_storage, scale)
    cond = np.logical_or(future_storage < boundaries[0], future_storage > boundaries[1])

    ###################

    future_price = next_price_2_rever_numba(current_time, price  , shocks_price) # adjust the price model
...
def action_value_numba(s, inflow, current_time, storage, q, price, V0, A, shocks_storage,
    ...
    if under_storage or over_storage:
        ...
        return pi_fun_numba(storage, inflow, current_time, price, q, scale) + next_state_value
...
for t in ts.index:
    ...
    pred_storage = model_storage(model_inflow, ts.loc[t-1,'inflow'] , model_evaporation, t, ts.loc[t-1,'storage'], ts.loc[t,'outflow'], 1)
...
def simulating_paths(loaded_policy, scale,T = 10000, t0 = time_0, storage_0 = ts_forecast.loc[time_0 - 1, 'storage'], price_0 = ts_forecast.loc[time_0 - 1, 'price'], inflow_0 = ts_forecast.loc[time_0 - 1, 'inflow']):
    ...
    for i in range(S):
        ...
        current_storage = model_storage_sensib(model_inflow_sensib,prev_inflow, model_evaporation, current_time, prev_storage, prev_inflow, scale)
        current_inflow  = model_inflow_sensib(prev_inflow, current_time,scale)
    ...
    for s in range(S):
        ...
        input_next_storage = [model_inflow, ts_forecast.loc[current_time - 1,'inflow'] , model_evaporation, current_time,
                                              storage[:,s], outflows, model_storage, scale] # Outflow changed from IV loop


        storage[:,s+1] = next_storage(*input_next_storage, z_s[:,s])
        inflow[:, s+1] = model_inflow(inflow[:,s], current_time, scale)
        if inflow[0,s+1] < 0: inflow[:, s+1] = 0 
        price[:,s+1] = next_price_2_rever_numba(current_time, price[:,s], z_p[:,s]) # adjust the price model
...
S = 80 # For the states
scales     = [0.5, 1, 2]
names      = ['sensib_inflow_I', 'sensib_inflow_q', 'sensib_inflow_inflow']
var_labels = {names[0]: 'Inventory (MCM)', names[1]: 'Outflow ($m^3$/s)', names[2]: 'Water inflow (MCM)'}
